...
export default function ChildProfile({ studentInit }) {
    ...
    >
      <motion.section variants={contVar} className="user">
        <motion.div variants={riseVar} className="avatar">
          <div className={ classNames("w-24 rounded-full ring-4 ring-offset-base-100 ring-offset-2", ringClr(student?.color))}>
            <ImageLoader
              src={student?.image}
        ...
        <motion.div
          variants={riseVar}
          className="backdrop-blur px-6 mt-4 rounded-3xl overflow-hidden"
        >
          <h1>{student?.name}</h1>
        </motion.div>
      </motion.section>
      <motion.section variants={contVar} className="button__sec">
        <Link href={`/child/info?id=${id}`}>
