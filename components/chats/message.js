    ...
    return (
      <motion.div
        ...
        className={classNames(
          "text-sm max-h-fit max-w-fit p-4 rounded-xl shadow-md",
          data?.type === "mine" && "ml-auto",
          data?.type === "other" ? "bg-white" : "bg-primary"
        )}
      >
        <p>{data?.message}</p>
        <p
          className={classNames(
            "text-xs float-right",
            data?.type === "mine" ? "text-yellow-700" : "text-gray-400"
          )}
        >
          {time}
        </p>
